    ...
    if request.method == "POST":
        ...
        if s_item_object:
            if current_user.can_sell(s_item_object):
                ...
                flash(
                    f'Congratulations! You sold {s_item_object.name} for {s_item_object.price}$ back to market', category='success')
            else:
                flash(
...
def register_page():
    ...
    if form.errors != {}:
        for err_msg in form.errors.values():
            flash(
                f'There was an error on creating user: {err_msg}', category='danger')
...
def login_page():
    ...
    if form.validate_on_submit():
        ...
        if attempted_user and attempted_user.check_password_correction(
            ...
            flash('Username does not exist or wrong password', category='danger')
